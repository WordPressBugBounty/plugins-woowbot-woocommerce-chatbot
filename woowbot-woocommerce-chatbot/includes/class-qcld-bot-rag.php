...
if ( ! class_exists( 'Qcld_Bot_Rag' ) ) {
    class Qcld_Bot_Rag{
		...
		public function check_api_endpoint() {
			...
			} else if (get_option('qcld_openai_enabled') == '1') {
				$this->api_key = get_option('qcld_open_ai_api_key');
		...
		public function generate_embedding($text) {
			if (!empty($this->api_key) && get_option('qcld_openai_enabled') == '1') {
		...
		{
			$apiKey = get_option('qcld_open_ai_api_key'); // Replace with option if needed
			global $wpdb;
		...
		{
			// $apiKey = get_option('qcld_open_ai_api_key');
			global $wpdb;
				...
				if ($p->post_type === 'product' && class_exists('WC_Product') && function_exists('wc_get_product')) {
					...
					if ( $_product && $_product->is_type( 'variable' ) ) {
						...
						foreach ( $variation_ids as $variation_id ) {
							$variation = wc_get_product( $variation_id ); // Load variation object
							
							// 1. Get the formatted name (e.g. "Product - Size, Color")
							$name = $variation->get_formatted_name(); 
							
							// 2. Get the current price and format it with currency symbol
							$price = wc_price( $variation->get_price() ); 

							// 3. Append to content in a clear format
